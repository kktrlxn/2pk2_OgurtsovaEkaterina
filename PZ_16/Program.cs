...
namespace PZ_16
{
    internal class Program
    {
        ...
        static byte buffs = 4; //количество усилений
        static int health = 3;  // количество аптечек
        static int hp = 50;// здоровье игрока
        static int dp = 5;// урон игрока
        static int dmg = 5; // урон врага
        static int healthEnemies = 30; // здоровье врага
        ...
        static List<int> buffsX = new List<int>();//координаты для бафов
        ...
        static void Enemy()
        {
            ...
            for (int j = 0; j < enemyY.Count; j++)
            {
                if (playerY == enemyY[j] && playerX == enemyX[j])
                {
                    hp -= dmg;
                    Text();
                    Count--;
        ...
        static void Buff()
        {
            for (int j = 0; j < buffsX.Count; j++)
            {
                if (playerY == buffsY[j] && playerX == buffsX[j])
                {
                    buffsX.RemoveAt(j);
                    buffsY.RemoveAt(j);
                    kolBuffForList--;
        ...
        static void Health()
        {
            for (int j = 0; j < healthX.Count; j++)
            {
                if (playerY == healthY[j] && playerX == healthX[j])
                {
                    hp = 50;
                    Text();
                    Count--;
            ...
            try
            {
                ...
                using (FileStream file2 = new FileStream(path2 + ".txt", FileMode.OpenOrCreate, FileAccess.Read))
                {
                    using (StreamReader reader1 = new StreamReader(file2))
                    {
                        ...
                        for (int i = 0; i < kolBuffForList; i++)//аптечка
                        {
